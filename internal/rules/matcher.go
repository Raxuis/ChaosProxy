...
func Compile(rules []config.Rule) (*Matcher, error) {
	...
	for index := range rules {
		method, pathPattern, err := parseExpression(rules[index].Match)
		if err != nil {
			issues = append(issues, fmt.Errorf("rules[%d] %q: %w", index, rules[index].Name, err))
			continue
		}
...
func (m *Matcher) Match(method, path string) *config.Rule {
	if m == nil {
		return nil
	}
